...
## 
## To run source script and call function do.analysis(outfilename = "wearables.txt")
...
createtable <- function(dataset) {
  ...
  tbl_x <- tbl_x %>% 
    ...
    rename_at(vars(starts_with("V")), funs({(function(x){tbl_names[substring(x,2),2]})(.)}))%>% 
...
## #############################################################################
## Main processing - do.analysis()
## #############################################################################
do.analysis <- function (outfilename = "wearables.txt") {
...
## create final dataset summarising mean and std by subject, activity and signal then save to disk
test %>%
  rbind(train) %>%
  group_by(subject, activity, signal) %>% 
  summarise(mean=mean(mean), std=mean(std)) %>%
  write.table(outfilename, row.name=FALSE)
}
